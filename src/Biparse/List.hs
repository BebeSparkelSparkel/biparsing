...
replicateBiparserT :: forall c s m em n u v.
  ( Monoid (SubState c s)
  , Monad m
  , MonadFail n
  )
...
-- | Takes
takeElementsWhile :: forall c s m em n ss.
   ( IsSequence ss
   , MonadFail m
   , MonadPlus m
   , MonadState s m
...
manyId :: forall c s m em n u v.
  ...
  => Biparser c s m em n u v
  -> Biparser c s Identity Void Identity [u] [v]
manyId = fix . many

-- | Iso version of 'many'
manyIso :: forall c s m em n a.
  Many c s m n
  => Iso c m em n s a
  -> Iso c m em n s [a]
manyIso = many
...
splitElem :: forall c s m em n ss.
  ( Take c s m n
  , Many c s m n
  , ss ~ SubState c s
  )
...
whileM :: forall c s m em n u v ss.
  ( MonadPlus m
  , MonadState s m
  , MonadWriter ss n
  , Alternative n
