...
spec = do
  fb "naturalBaseTen"
    (naturalBaseTen :: Iso LineColumn (FM Text) ErrorPosition IO (Position Text) Word)
    (\f -> do
      it "success" do
  ...
  fb "intBaseTen"
    (intBaseTen :: Iso LineColumn (FM Text) ErrorPosition IO (Position Text) Int)
    (\f -> do
      it "success" do
        f "1"     `shouldBe` Right (1,   Position 1 2 mempty)
        f "123"   `shouldBe` Right (123, Position 1 4 mempty)
  ...
  fb "realBaseTen"
    (realBaseTen :: Iso LineColumn (FM String) ErrorPosition IO (Position String) Double)
    (\f -> do
      it "whole" do
        f "1"    `shouldBe` Right (1,   Position 1 2 mempty)
        f "123"  `shouldBe` Right (123, Position 1 4 mempty)
