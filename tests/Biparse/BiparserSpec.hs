...
spec = do
  describe "one" do
    describe "IdentityState" do
      fb "id"
        (one :: Iso IdentityState IO IOException IO String Char)
        (\f -> do
          it "one use" do
            x <- f "abc"
            x `shouldBe` ('a',"bc")
      ...
      fb "tuple"
        ((,) <$> one `upon` fst <*> one `upon` snd :: Iso IdentityState IO IOException IO String (Char,Char))
        (\f -> do
          it "used twice" do
            x <- f "abc"
            x `shouldBe` (('a','b'),"c")
    ...
    fb "LineColumn"
      (one :: Iso LineColumn (FM Text) ErrorPosition IO (Position Text) Char)
      (\f -> do
        it "empty" do
  ...
  describe "split" do
    ...
    let takeTwo :: Iso IdentityState IO IOException IO String String
        takeTwo = split do
          x <- get
    ...
    fb "IdentityState"
      --(takeTwo @IdentityState @IO @IOException @String)
      takeTwo
      (\f -> do
    ...
    --fb "LineColumn"
    --  (takeTwo :: Iso LineColumn (FM String) ErrorPosition IO (Position String) String)
    --  (\f -> do
...
--  describe "peek" do
--    fb "simple"
--      (peek one :: Iso IdentityState IO IOException IO String Char)
--      (\f -> do
--        it "none consumed" do
--          x <- f "abc"
--          x `shouldBe` ('a',"abc")
...
--      fb "IdentityState"
--        (peek (takeUni 'x') <|> takeUni 'a' :: Iso IdentityState IO IOException IO String Char)
--        (\f -> do
--          it "take first" do
--            x <- f "xa"
--            x `shouldBe` ('x',"xa")
...
--
--      fb "LineColumn"
--        (peek (takeUni 'x') <|> takeUni 'a' :: Iso LineColumn (FM String) ErrorPosition IO (Position String) Char)
--        (\f -> do
...
--
--  describe "try" do
--    let bp = (try $ one <* take 'b' :: Biparser IdentityState (Seq Char) IO IOException IO Char Char)
--        f = runForward bp
--        b = runBackward bp
--
--    describe "forward" do
...
--  describe "isNull" do
--    fb "IdentityState"
--      (isNull :: ConstU IdentityState String Identity Void Identity [()] Bool)
--      (\f -> do
...
--
--    fb "LineColumn"
--      (isNull :: ConstU LineColumn (Position String) Identity Void Identity [()] Bool)
--      (\f -> do
...
--    fb "LineColumn"
--      (breakWhen' $ stripPrefix "ab" :: Iso LineColumn (FM String) ErrorPosition IO (Position String) String)
--      (\f -> do
--        it "empty" $ limit $
--          f "" `shouldSatisfy` errorPosition 1 1
--
...
--
--    fb "IdentityState"
--      (breakWhen' $ stripPrefix "ab" :: Iso IdentityState IO IOException IO String String)
--      (\f -> do
--        it "empty" $ limit $
